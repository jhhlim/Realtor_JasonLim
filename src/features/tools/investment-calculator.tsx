  ...
  return (
    <div className="space-y-8">
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2 sm:col-span-2">
              <Label htmlFor="inv-address">Property / area (optional)</Label>
              <Input
                id="inv-address"
                value={propertyAddress}
                onChange={(e) => setPropertyAddress(e.target.value)}
                placeholder="e.g. Milpitas duplex, Berryessa SFH"
              />
            </div>
            {[
      ...
      <AiAnalyzeSection
        tool="investment"
        inputs={{
          propertyAddress: propertyAddress || "Silicon Valley investment property",
          purchasePrice,
          grossRent,
          vacancyPct,
        ...
        computedMetrics={{
          noi,
          annualCashFlow,
          capRatePercent: Number(capRate.toFixed(2)),
          cashOnCashPercent: Number(coc.toFixed(2)),
        }}
        label="Get Jason's investment analysis"
      />
    </div>
  );
}
