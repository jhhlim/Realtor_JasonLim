...
export function AffordabilityCalculator() {
  ...
  const [targetCity, setTargetCity] = React.useState("San Jose / Berryessa");
  ...
  const computedMetrics = React.useMemo(
    () => ({
      ...
      maxMonthlyHousing: result.maxMonthlyHousing,
      maxLoanAmount: result.maxLoanAmount,
      targetDtiPercent: dti,
      downPaymentPercent,
      annualRate: rate,
    }),
    [result, dti, downPaymentPercent, rate],
  );
  ...
  const inputs = React.useMemo(
    () => ({
      ...
      monthlyDebts,
      targetCity,
      market: "Silicon Valley / South Bay",
    }),
    [annualIncome, monthlyDebts, targetCity],
  );
  ...
  return (
    <div className="space-y-8">
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2 sm:col-span-2">
              <Label htmlFor="target-city">Target area</Label>
              <Input
                id="target-city"
                value={targetCity}
                onChange={(e) => setTargetCity(e.target.value)}
                placeholder="e.g. Berryessa, Milpitas, Cupertino"
              />
            </div>
            <div className="space-y-2 sm:col-span-2">
        ...
        <Card className="bg-gradient-to-br from-slate-soft to-background dark:from-card">
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <p className="pt-2 text-muted-foreground">
              Heuristic using {dti}% DTI, {downPaymentPercent}% down, and illustrative
              tax/insurance assumptions. Lenders underwrite with credit, reserves, and
              program overlays.
            </p>
          </CardContent>
        </Card>
      ...
      <AiAnalyzeSection
        ...
        label="Get Jason's affordability analysis"
      />
    </div>
  );
}
